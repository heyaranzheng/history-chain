...
use crate::req_resp::{self, Request, RequestWorker, Response, WorkReceiver, Worker};
...
pub trait AsyncHandler {
    async fn handle(&self, payload: Payload) -> Result<Payload, HError>;
...
/// a register structure for async handlers.
...
impl AsyncRegister {
    ...
    /// handle a payload with relevent handler.
    /// if there is no relevent handler, return an error.
    pub(super) async fn handle(&self, payload: Payload) -> Result<Payload, HError> {
        let payload_type = PayloadTypes::from_payload(&payload);
        if let Some(handler) = self.handlers.get(&payload_type){
            let payload_ret = handler(payload).await?;
            Ok(payload_ret)
        }else {
            Err(
                HError::Message { 
                    message:  format!("No handler for payload type: {:?}", payload_type),
                }
            )
        }
    }
...
/// the async register for payloads.
/// the clone function will create a new register, by cloning the inner register:
/// Arc<Mutex<AsyncRegister>>
...
///clone the inner rigister: Arc<Mutex<AsyncRegister>>
...
impl AsyncHandler for AsyncPayloadHandler {
    ...
    async fn handle(&self, payload: Payload) -> Result<Payload, HError> {
        ...
        if let Some(handler) = register.handlers.get(&payload_type) {
            //return the result
    ...
    where
        ...
        Fut: Future<Output = Result<Payload, HError>> + Send + Sync + 'static,
    {
        let box_pinned_handler = Box::new(move |payload: Payload| {
    ...
    async fn spawn_run(
        ...
        let (request_worker, mut request_reciver) = req_resp::create_channel::<Payload>(capacity);
...
impl HandlerContext {
    ...
    pub fn get_request_worker(&self, payload_type: PayloadTypes) -> Option<RequestWorker<Payload>> {
        self.request_wokers.get(&payload_type).cloned()
    }

    async fn send(&self, payload: Payload) -> Result<Response<Payload>, HError> {
        let payload_type = PayloadTypes::from_payload(&payload);
...
impl SyncHandler for PayloadHandler {
    ...
    async fn spawn_run(
        ...
        let (work_request, mut work_receiver) = req_resp::create_channel::<Payload>(capacity);
    ...
    fn handle(&self, payload: Payload) -> Result<Payload, HError> {
        ...
        if let Some(handler) = self.register.handlers.get(&payload_type) {
            result = handler(payload)
        } else {
            return Err(HError::NetWork {
                message: format!("No handler for payload type: {:?}", payload_type).to_string(),
            });
        }
        return result;
    }
...
mod tests {
    ...
    async fn test_payload_handler() {
        ...
        let handle = |payload| -> Result<Payload, HError> {
        ...
        let woker_result = handler.spawn_run(1, canc_token.clone()).await;
        ...
        let response_result = Request::send(Payload::Empty, request_worker.clone()).await;
    ...
    async fn test_async_payload_handler() {
        ...
        let async_handle = |payload| async {
        ...
        // spawn the handler task, get the request_worker for communication
        let request_worker_result = handler.spawn_run(1, canc_token).await;
        ...
        // make a request to the handler task
        let result = Request::send(Payload::Empty, request_worker.clone()).await;
