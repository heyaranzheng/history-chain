            ...
            if let Some((tcp_stream, src_addr_str)) = receiver.recv().await{
                ...
                loop {
                    ...
                    if save_counter < MAX_CONNECTIONS {
                        ...
                        //No, the counter is down, wait for a litte time
                ...
                return Err(
                    HError::NetWork { message: format!("tcp_listen_with_thread error") }
                );
            }
